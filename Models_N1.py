...
class ResNet101_depth_with_multi(torch.nn.Module):
  ...
  def forward(self, x):
    ...
    #why is two prediction layer needed? -> why cant i go from 256 to output channels)
    normal = self.predict_normal2(self.predict_normal1(p2))
    #might have to separate each layers
    ...
    features = F.avg_pool2d(c5,7).view(-1,2048) #extracting feature map. 
