...
class UNet_with_attention(torch.nn.Module):
  ...
  def __init__(self, num_classes, attention=False, normalize_attn=False):
    ...
    if self.attention:
        ...
        self.attn4 = LinearAttentionBlock(in_features= 1024, normalize_attn=normalize_attn)
        self.classifier = torch.nn.Linear(1024, num_classes)
  ...
  def forward(self, x): 
    ...
    e1 = self.encoder2(p2) #expansive -> doesnt it require decoder layer? 
    f1,f2,f3,f4 = None, None, None, None
    if self.attention:
        g = F.avg_pool2d(e1,7) 
        f1, g1 = self.attn1(self.projector(l1),g) 
        f2, g2 = self.attn2(l2,g)
        f3, g3 = self.attn3(self.projector(l3),g)
        f4, g4 = self.attn4(self.projector(l4),g)
        g = g1 + g2 + g3 + g4  
        output = self.classifier(g)
    else:
        g = F.avg_pool2d(e1,7).view(-1,1024) #extracting feature map.
        output = self.classifier(g)
