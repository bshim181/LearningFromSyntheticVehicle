...
class AutoAugment(object):
    def __init__(self):
        ...
        self.applied_policy_first = None
        self.applied_policy_second = None
        self.magnitude_first = None 
        self.magnitude_second = None 
        self.policy_range = None 
        self.range_num = None

    def __call__(self, img):
        policy = self.policies[random.randrange(len(self.policies))]
        self.policy_range = policy
        img = apply_policy(self, img, policy)
        return img

    def return_applied_policy(self):
        return self.applied_policy_first, self.magnitude_first, self.applied_policy_second, self.magnitude_second, self.policy_range, self.range_num
...
def apply_policy(self,img, policy):
    num = random.random()
    if num < policy[1]:
        img = operations[policy[0]](img, policy[2])
        self.applied_policy_first = operations[policy[0]]
        self.magnitude_first = policy[2] 
    if num < policy[4]:
        img = operations[policy[3]](img, policy[5])
        self.applied_policy_second = operations[policy[3]]
        self.magnitude_second = policy[5]
